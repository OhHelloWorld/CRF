...
angular.module('patientOverview', ['main'])
  .controller('patientOverviewController', ['$scope', '$http', '$state', 'localStorageService', function($scope, $http, $state, localStorageService) {
    ...
    function displayTongue(response) {
    ...
    function displayTreat(response) {
    ...
    function displayMedicine(response) {
    ...
    function getFollowDateList() {
      $http({
        method: 'GET',
        url: '/api/patient/followDate/' + sessionStorage.getItem('patientId')
      }).then(function success(response) {
        $scope.followDateList = response.data;
      });
    }
    ...
    $scope.followClick = function(followDate) {
      $http({
        method: 'GET',
        url: '/api/boneDensity/follow?date=' + formatFollowDate(followDate)
      }).then(function success(response) {
        if (response.data) {
          $scope.showBoneFollow = true;
          displayBone(response);
        } else {
          $scope.showBoneFollow = false;
        }
      ...
      $http({
        method:'GET',
        url:'/api/fourDiagnosticInfor/follow?date='+formatFollowDate(followDate)
      }).then(function success(response){
        if(response.data){
          $scope.showFourDiaFollow = true;
          displayFour(response);
        }else{
          $scope.showFourDiaFollow = false;
        }
      });

      $http({
        method:'GET',
        url:'/api/medicine/follow?date='+formatFollowDate(followDate)
      }).then(function success(response){
        if(response.data){
          $scope.showChineseFollow = true;
          displayMedicine(response);
        }else{
          $scope.showChineseFollow = false;
        }
      });
      ...
      $http({
        method:'GET',
        url:'/api/physical/follow?date='+formatFollowDate(followDate)
      }).then(function success(response){
        if(response.data){
          $scope.showPhyFollow = true;
          displayPhy(response);
        }else{
          $scope.showPhyFollow = false;
        }
      });

      $http({
        method:'GET',
        url:'/api/tonguePulse/follow?date='+formatFollowDate(followDate)
      }).then(function success(response){
        if(response.data){
          $scope.showTongueFollow = true;
          displayTongue(response);
        }else{
          $scope.showTongueFollow = false;
        }
      });

      $http({
        method:'GET',
        url:'/api/treatment/follow?date='+formatFollowDate(followDate)
      }).then(function success(response){
        if(response.data){
          $scope.showTreatFollow = true;
          displayTreat(response);
        }else{
          $scope.showTreatFollow = false;
        }
      });
    };

    function formatFollowDate(input) {
